    ...
    <div className="two-col">
      ...
      <Card>
        ...
        <form className="form-grid" onSubmit={e=>{e.preventDefault();save('/opening/customer-balance',customer,()=>setCustomer({date:today(),customer_id:'',balance_type:'Due',amount:''}),'Customer opening balance saved.')}}>
          <label>Date (DD/MM/YYYY)<DateField required value={customer.date} onChange={date=>setCustomer({...customer,date})}/></label>
      ...
      <Card>
        ...
        <form className="form-grid" onSubmit={e=>{e.preventDefault();save('/opening/source-balance',source,()=>setSource({date:today(),source_id:'',balance_type:'Payable',amount:''}),'Source opening balance saved.')}}>
          <label>Date (DD/MM/YYYY)<DateField required value={source.date} onChange={date=>setSource({...source,date})}/></label>
          <label>Source<select required value={source.source_id} onChange={e=>setSource({...source,source_id:e.target.value})}><option value="">Select source</option>{sources.map(s=><option key={s.id} value={s.id}>{s.name} — {s.source_type}</option>)}</select></label>
