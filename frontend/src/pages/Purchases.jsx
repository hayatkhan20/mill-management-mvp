...
export default function Purchases(){
  ...
  return <>
    ...
    <Card className="section-card-below">
      <div className="history-toolbar"><h3>Purchase History</h3><div className="history-filter"><DateField value={historyDate} onChange={applyDate}/><button type="button" className="secondary" onClick={showAll}>Show All</button></div></div>
      <div className="history-stats purchase-history-stats">
        <div><span>Wheat</span><strong>{num(summary.wheatKg)} KG</strong></div>
        <div><span>Bardana</span><strong>{num(summary.bardanaBags)} Bags</strong></div>
        <div><span>Wheat Cost</span><strong>{money(summary.wheatCost)}</strong></div>
        <div><span>Bardana Cost</span><strong>{money(summary.bardanaCost)}</strong></div>
        <div><span>Total Purchase</span><strong>{money(summary.total)}</strong></div>
      </div>
      {combined.length?<div className="table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Source</th><th>Quantity</th><th>Rate</th><th>Total</th></tr></thead><tbody>
