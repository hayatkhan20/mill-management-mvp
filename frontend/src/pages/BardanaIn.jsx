 ...
 return <>
  ...
  <div className="two-col">
   ...
   <Card><h3>Recent Bardana Purchases</h3>{rows.length?<div className="table-wrap"><table><thead><tr><th>Date</th><th>Source</th><th>Bags</th><th>Rate/Bag</th><th>Total</th></tr></thead><tbody>
    {rows.slice(0,15).map(r=><tr key={r.id}><td>{r.date}</td><td><strong>{r.source_name}</strong><small>{r.source_type}</small></td><td>{num(r.quantity)}</td><td>{money(r.rate_per_bag)}</td><td>{money(r.total_cost)}</td></tr>)}
