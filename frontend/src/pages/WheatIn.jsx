...
export default function WheatIn(){
 ...
 return <>
  ...
  <div className="two-col">
   ...
   </form>{!sources.length&&<small>Add a source from the Sources tab before recording wheat.</small>}</Card>
   ...
   <Card><h3>Recent Wheat Purchases</h3>{rows.length?<div className="table-wrap"><table><thead><tr><th>Date</th><th>Source</th><th>Bags</th><th>Wheat KG</th><th>Wheat Cost</th><th>Bardana Cost</th><th>Total</th></tr></thead><tbody>
    {rows.slice(0,15).map(r=><tr key={r.id}><td>{r.date}</td><td><strong>{r.source_name}</strong><small>{r.source_type}</small></td><td>{num(r.bags)}</td><td>{num(r.total_kg)}</td><td>{money(r.total_cost)}</td><td>{money(r.bardana_cost)}</td><td><strong>{money(r.purchase_total)}</strong></td></tr>)}
