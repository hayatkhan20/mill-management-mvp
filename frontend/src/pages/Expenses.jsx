...
export default function Expenses(){
 ...
 return <>
  ...
  <Card>
   <div className="section-title"><h3>{t('monthlyExpenseSummary','Monthly Expense Summary')}</h3><input type="month" value={month} onChange={e=>setMonth(e.target.value)}/></div>
  ...
  <div className="two-col" style={{marginTop:18}}>
   <Card>
    <h3>{editingId?t('editOtherExpense','Edit Other Expense'):t('addOtherExpense','Add Other Expense')}</h3>
    <form className="form-grid" onSubmit={submit}>
     ...
     <label>{t('category','Category')}
      <select required value={categoryChoice} onChange={e=>{setCategoryChoice(e.target.value);if(e.target.value!=='custom')setCustomCategory('')}}>
       ...
       <option value="custom">{t('customCategory','One-time / Custom Category')}</option>
      </select>
     </label>

     {categoryChoice==='custom'&&<label className="span-2">{t('categoryName','Category Name')}
     ...
     <label>{t('amount','Amount')}<input required type="number" min="0.01" step="0.01" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})}/></label>
     <label>{t('note','Note')}<input value={form.note} onChange={e=>setForm({...form,note:e.target.value})} placeholder="Optional"/></label>

     <div className="span-2">
      <button className="primary">{editingId?t('saveChanges','Save Changes'):t('addExpense','Add Expense')}</button>
   ...
   <Card>
    <h3>{t('otherExpenses','Other Expenses')}</h3>
    {data?.manual?.length?<div className="table-wrap"><table><thead><tr><th>{t('date','Date')}</th><th>{t('category','Category')}</th><th>{t('amount','Amount')}</th><th>{t('note','Note')}</th><th></th></tr></thead><tbody>
