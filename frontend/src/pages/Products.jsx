...
export default function Products(){
  ...
  return <>
    ...
    <div className="two-col products-layout">
      <Card>
        <h3>{t('addProduct','Add Product')}</h3>
        <form onSubmit={add} className="form-grid">
          <label className="span-2">{t('product','Product')}<input required value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Bran"/></label>
          <div className="span-2"><button className="primary">{t('addProduct','Add Product')}</button></div>
        </form>
      </Card>
      <Card>
        <h3>{t('products','Products')}</h3>
        {finished.length?<div className="table-wrap"><table><thead><tr><th>{t('product','Product')}</th><th>{t('status','Status')}</th><th></th></tr></thead><tbody>
          {finished.map(p=><tr key={p.id}><td><strong>{p.name}</strong></td><td>{p.is_active?t('active','Active'):t('inactive','Inactive')}</td><td><button className="secondary small" onClick={()=>toggle(p)}>{p.is_active?'Deactivate':'Activate'}</button></td></tr>)}
