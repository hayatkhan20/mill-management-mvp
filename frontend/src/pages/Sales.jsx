...
export default function Sales(){
 ...
 const updateItem=(idx,key,value)=>setItems(prev=>prev.map((it,i)=>{
   ...
   if(key==='bag_size'||key==='bags') next.total_kg=Number(next.bag_size||0)*Number(next.bags||0)||'';
   return next;
 }));
 ...
 const addCustomer=async e=>{
   ...
   try{
     ...
     setNewCustomer(blankCustomer());setShowAddCustomer(false);setSuccess(`${created.name} added and selected for this bill.`);
 ...
 {items.map((it,idx)=><div className="sale-row" key={idx}>
   ...
   <input readOnly value={it.total_kg||''} placeholder="0"/>
   <input required type="number" min="0.01" step="0.01" value={it.rate} onChange={e=>updateItem(idx,'rate',e.target.value)}/>
   <div className="amount-cell">{money(Number(it.bags||0)*Number(it.rate||0))}</div>
