...
export default function Sources(){
 ...
 const pay=async e=>{e.preventDefault();setError('');setSuccess('');try{await api.post('/source-payments',{source_id:selected.id,...payment});setSuccess('Source payment recorded.');await open(selected.id);load()}catch(e){setError(e.message)}};
 ...
 return <>
  ...
  {selected&&<div className="modal"><div className="customer-modal">
   <div className="modal-actions">{!editing&&<button className="secondary" onClick={()=>setEditing(true)}>Edit Source</button>}<button className="secondary" onClick={()=>{setSelected(null);setEditing(false)}}>Close</button></div>
